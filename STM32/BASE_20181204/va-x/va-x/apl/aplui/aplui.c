...
static void drvpbt_callback(uint32_t button, int32_t event);
...
static MDLSTRG_DATA_ROOM_STS_T uirst_value;
/*
 * 内部変数
 */
// イベント送信先
...
void aplui_task(intptr_t exinf)
{
    ...
    while (true) {
        ...
        DBGLOG1("aplui_task() received: (msg = %d).", event->code);
        
        process_event(event);

        switch (event->code) {

        case APLEVT_UI_PBT_STOP: {
            DBGLOG0("APLEVT_UI_PBT_STOP");
            drvpbt_stop();
            break;
        }
...
{
//    DBGLOG2("led_set_pattern LED%d, ptn%d", led, pattern);
...
//        led_set_pattern(DRVLED_LED_FRONT_YGREEN, LEDPATTERN_ON);
    ...
    case APLEVT_REGISTRATION_PREPARE_REQ:
        if((event->error) == 1){
            DBGLOG0("APLEVT_REGISTRATION_PREPARE_OK.");
            led_turn_off_all();
            led_set_pattern(DRVLED_LED_FRONT_BLUE, LEDPATTERN_BLINK_8HZ);
            dly_tsk(1000);
            buz_set_pattern(BUZPATTERN_ACCEPT);
        }else if((event->error) == 2){
            DBGLOG0("APLEVT_REGISTRATION_PREPARE_NG.");
            led_turn_off_all();
            led_set_pattern(DRVLED_LED_FRONT_BLUE, LEDPATTERN_BLINK_8HZ);
            dly_tsk(1000);
...
//        buz_set_pattern(BUZPATTERN_ACCEPT);
    ...
    case APLEVT_AUTH_COMPLETE:
        DBGLOG0("APLEVT_AUTH_COMPLETE.");
        if (!event->error) {
            led_turn_off_all();
            led_set_pattern(DRVLED_LED_FRONT_BLUE, LEDPATTERN_ON);
            buz_set_pattern(BUZPATTERN_ACCEPT);
...
//            led_set_pattern(DRVLED_LED_FRONT_GREEN, LEDPATTERN_ON_AFTER_3000MS);
    ...
    case APLEVT_AUTH_ERROR:
        DBGLOG0("APLEVT_AUTH_ERROR.");
            led_turn_off_all();
            led_set_pattern(DRVLED_LED_FRONT_RED, LEDPATTERN_ON);
            dly_tsk(1000);
            buz_set_pattern(BUZPATTERN_REJECT);
        break;
...
//            led_set_pattern(DRVLED_LED_FRONT_GREEN, LEDPATTERN_ON_AFTER_3000MS);
            buz_set_pattern(BUZPATTERN_ACCEPT);
        } else if(event->error == 3){
            led_turn_off_all();
            led_set_pattern(DRVLED_LED_FRONT_RED, LEDPATTERN_BLINK_8HZ);
            dly_tsk(1000);
//            led_set_pattern(DRVLED_LED_FRONT_GREEN, LEDPATTERN_ON_AFTER_3000MS);
            buz_set_pattern(BUZPATTERN_REJECT);
        }
...
//        led_set_pattern(DRVLED_LED_FRONT_GREEN, LEDPATTERN_ON_AFTER_3000MS);
    ...
    case APLEVT_BATTERY_LOW:
        ...
        break;
    case APLEVT_LINKEY_READY:
        DBGLOG0("APLEVT_LINKEY_READY.");
        led_turn_off_all();
        led_set_pattern(DRVLED_LED_FRONT_GREEN, LEDPATTERN_ON);
        break;
    case APLEVT_LINKEY_COMPLETE:
        DBGLOG0("APLEVT_LINKEY_COMPLETE.");
        led_turn_off_all();
        led_set_pattern(DRVLED_LED_FRONT_GREEN, LEDPATTERN_ON);
        break;
    case APLEVT_LOCK_ERROR:
        ...
        */
        break;
    case APLEVT_REG_CARD_REG_VACANCY_READY:
        //DBGLOG0("APLEVT_REG_CARD_REG_VACANCY_READY");
        led_turn_off_all();
        led_set_pattern(DRVLED_LED_FRONT_YGREEN, LEDPATTERN_BLINK_2HZ);
        break;
    case APLEVT_REG_CARD_REG_VACANCY_COMPLETE:
        //DBGLOG0("APLEVT_REG_CARD_REG_VACANCY_COMPLETE");
        led_turn_off_all();
        led_set_pattern(DRVLED_LED_FRONT_YGREEN, LEDPATTERN_ON);
        break;
    case APLEVT_REG_CARD_VACANCY_READY:
        //DBGLOG0("APLEVT_REG_CARD_VACANCY_READY");
        //空室カード登録
        led_turn_off_all();
        led_set_pattern(DRVLED_LED_FRONT_BLUE, LEDPATTERN_BLINK_2HZ);
        break;
    case APLEVT_REG_CARD_VACANCY_COMPLETE:
        //DBGLOG0("APLEVT_REG_CARD_VACANCY_COMPLETE");
        led_turn_off_all();
        led_set_pattern(DRVLED_LED_FRONT_BLUE, LEDPATTERN_ON);
        break;
    case APLEVT_REREG_CARD_REG_VACANCY_READY:
        //DBGLOG0("APLEVT_REREG_CARD_REG_VACANCY_READY");
        led_turn_off_all();
        led_set_pattern(DRVLED_LED_FRONT_RED, LEDPATTERN_BLINK_2HZ);
        break;
    case APLEVT_REREG_CARD_REG_VACANCY_COMPLETE:
        //DBGLOG0("APLEVT_REREG_CARD_REG_VACANCY_COMPLETE");
        led_turn_off_all();
        led_set_pattern(DRVLED_LED_FRONT_RED, LEDPATTERN_ON);
        break;
    default:
 ...
 *
 */
void drvpbt_callback(uint32_t button, int32_t event)
{
    //
    DBGLOG2("drvpbt_callback: b:%d, e:%d", button, event);
	APLEVT_EXTRA_DATA_T extra_data;
    ER er = E_OK;
    
    switch (event) {
    case DRVPBT_EVT_PRESS:	// 初期化完了
//        DBGLOG0("callback: MDLAUTH_EVT_INITIALIZE_COMPLETE");
        DBGLOG2("EVT_PRESS: b:%d, e:%d", button, event);
        extra_data.reg_prep_req.index = 1;
        aplevt_send_event(APLEVT_BLE_ENABLE, 0, &extra_data, s_event_dest);
//        aplevt_send_event(APLEVT_BLE_ENABLE, 0, NULL, s_event_dest);
//        DBGLOG2("EVT_FOURPRESS: b:%d, e:%d", button, event);
/*
        MDLSTRG_REQUEST_T ROOM_STS_WREQ = {
            .data_type = MDLSTRG_DATA_TYPE_ROOM_TYPE,
            .request_type = MDLSTRG_REQ_TYPE_WRITE,
            .data = (intptr_t)&uirst_value,
            .size = sizeof(uirst_value),
        };
        mdlstrg_request(&ROOM_STS_WREQ, strg_callback);
        er = twai_flg(FLG_APLUI, FLGPTN_MDLSTRG_REQ_COMPLETE, TWF_ANDW, &(FLGPTN){0}, 2000);
        assert(er == E_OK);
        
*/
        break;

    case DRVPBT_EVT_THREEPRESS:
        DBGLOG2("EVT_THREEPRESS: b:%d, e:%d", button, event);
/*      // 空室カード登録
...
*/
        ...
        aplevt_send_event(APLEVT_ICC_REREGIST_REQ, 0, &extra_data, s_event_dest);
        break;
        
    case DRVPBT_EVT_FOURPRESS:
        DBGLOG2("EVT_FOURPRESS: b:%d, e:%d", button, event);
/*
        MDLSTRG_REQUEST_T ROOM_STS_WREQ = {
            .data_type = MDLSTRG_DATA_TYPE_ROOM_TYPE,
...
*/
    default:
        DBGLOG2("EVT_RELEASE: b:%d, e:%d", button, event);
//        aplevt_send_event(APLEVT_BLE_ENABLE, 0, NULL, s_event_dest);
        break;
    }
...
{
    DBGLOG1("APLUI_STRG CALLBACK: %d", event);

    ER er = E_OK;

    switch (event) {
    case MDLSTRG_EVT_REQUEST_COMPLETE:
        er = set_flg(FLG_APLUI, FLGPTN_MDLSTRG_REQ_COMPLETE);
        assert(er == E_OK);
        break;
    default:
        break;
