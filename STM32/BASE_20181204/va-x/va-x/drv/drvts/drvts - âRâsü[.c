...
//	syslog(LOG_NOTICE, "drvts_task() starts .");
    ...
    int32_t event = DRVTS_EVT_RELEASE;
    int release_detected = false;
...
#endif
            if (pin1 || pin2) {
//                event = DRVTS_EVT_TOUCH1;
                DBGLOG0("DRVTS_EVT_TOUCH1 .");
            } else if (pin1 && pin2) {
                event = DRVTS_EVT_TOUCH2;
                DBGLOG0("DRVTS_EVT_TOUCH2 .");
            } else if (!pin1 && !pin2) {
            ...
            DBGLOG2("last_event=%d, event=%d", last_event, event);
//            syslog(LOG_NOTICE, "drvts_task() TMOUT pin1=%d, pin2=%d", pin1, pin2);
//            DBGLOG2("drvts_task() TMOUT pin1=%d, pin2=%d", pin1, pin2);
            
//            if (((event == DRVTS_EVT_TOUCH2)||(last_event == DRVTS_EVT_TOUCH2))&&(event != last_event)) {
            if (event != last_event) {
                if (s_callback) {
//                    syslog(LOG_NOTICE, "drvts_task() callback %d", event);
                    DBGLOG1("drvts_task() callback %d", event);
//                    if(release_detected == true)
//                    {
                        s_callback(event);
