...
void aplauth_task(intptr_t exinf)
{
    ...
    while (true) {
        ...
        case APLEVT_AUTH_PREPARE_REQ:
            ...
            break;
        case APLEVT_REGISTRATION_PREPARE_REQ:
        	DBGLOG0("dbgAPLEVT_REGISTRATION_PREPARE_REQ");
...
void mdlauth_callback(int event, intptr_t opt1, intptr_t opt2)
{
    ...
    case MDLAUTH_EVT_REGISTRATION_COMPLETE:	// 登録完了
        ...
        break;
    case MDLAUTH_EVT_REGISTRATION_RETRYING:			// 登録リトライ
	    DBGLOG0("callback: MDLAUTH_EVT_REGISTRATION_RETRYING");
	    aplevt_send_event(APLEVT_REGISTRATION_PREPARE_REQ, opt1, NULL, aplauth_event);
        //LED点灯用イベント
        aplevt_send_event(APLEVT_REGISTRATION_PREPARE_REQ, opt1, NULL, s_receiver_func);
	   break;
