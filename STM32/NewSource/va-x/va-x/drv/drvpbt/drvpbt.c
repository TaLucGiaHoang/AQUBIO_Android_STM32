...
static const TMO CHATTER_TIMEOUT = 2000;
...
//    DBGLOG1("isr %d", pinno);
    ...
    if (!(drvcmn_gpio_pin_get(&GPIO_PINS[0]))) {
        ...
        DBGLOG1("isr_msg_prs %d", msg);
    } else {
        msg = DRVPBT_EVT_RELEASE;
        DBGLOG1("isr_msg_rls %d", msg);
    }
...
void drvpbt_task(intptr_t exinf)
{
    ...
    while (true) {
        ...
        if (er_rcv == E_TMOUT) {
            ...
            if (s_callback) {
                DBGLOG1("drvpbt_task() pending_event: %d.", pending_event);
                s_callback(0, pending_event);
                cnt = 0;
                if (event != pending_event) {
                    DBGLOG1("drvpbt_task() event: %d.", event);
                    s_callback(0, event);
                }
            }
            pending_event = 0;
            ...
            case DRVPBT_EVT_RELEASE:	/* リリース */
                DBGLOG1("drvpbt_task() msg: %d.", msg);
                pending_event = msg;
                
                if (event != pending_event) {
                    DBGLOG2("drvpbt_task() event: %d, cnt: %d.", event, cnt);
                    cnt++;
                    if(cnt > card_thresh){
                        DBGLOG1("3times event: %d.", event);
                        s_callback(0, DRVPBT_EVT_THREEPRESS);
                        cnt = 0;
                    }else if (cnt > card_threshf){
                        s_callback(0, DRVPBT_EVT_FOURPRESS);
                        cnt = 0;
                    }
                }
                break;
