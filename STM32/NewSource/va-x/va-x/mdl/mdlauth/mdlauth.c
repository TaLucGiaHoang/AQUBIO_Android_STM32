...
// 登録リミット回数
static const int AUTH_FINGER_LIMIT = 3;
...
// 登録動作
enum {
	MDLAUTH_REG_MATCH = 0,// 登録OK
	MDLAUTH_REG_FIN_OK,		// 指先OK　次の撮影へ
	MDLAUTH_REG_FIN_NG,	// 指先NG
	MDLAUTH_REG_MISSMATCH,	// 登録NG
};
...
void mdlauth_task(intptr_t exinf)
{
    ...
    int callback_type = 0;
    intptr_t result = 0;
    s_context.finger_count = 0;
    s_context.registerd_count = 0;
    while (true) {
        ...
        case MSG_TS_DETECTED: { // タッチセンサー検出
            ...
            assert(s_context.callback);
            DBGLOG1("s_context.mode:0x%x", s_context.mode);
//            s_context.callback(MDLAUTH_EVT_TS_DETECT, 0, 0);
            ...
            if (s_context.mode == MDLAUTH_MODE_REGISTRATION) {
                ...
                if(result == MDLAUTH_REG_MATCH){
                	DBGLOG0("dbg_MDLAUTH_REG_MATCH");
                    callback_type = MDLAUTH_EVT_REGISTRATION_COMPLETE;
                    s_context.finger_count = 0;
                    s_context.registerd_count = 0;
                    stop();
                }else if(result == MDLAUTH_REG_FIN_OK){
                	DBGLOG0("dbg_MDLAUTH_REG_FIN_OK");
                    s_context.finger_count = 0;
                    s_context.registerd_count++;
                    callback_type = MDLAUTH_EVT_REGISTRATION_RETRYING;
                    s_context.state = STATE_INITIALIZED;
                } else if(result == MDLAUTH_REG_FIN_NG){
                	DBGLOG0("dbg_MDLAUTH_REG_FIN_NG");
                    s_context.finger_count++;
                    if (s_context.finger_count < AUTH_FINGER_LIMIT) {
                    	callback_type = MDLAUTH_EVT_REGISTRATION_RETRYING;
                        s_context.state = STATE_INITIALIZED;
                    } else {
                    	DBGLOG0("dbg_MDLAUTH_REG_FIN_NG*3");
                        callback_type = MDLAUTH_EVT_REGISTRATION_COMPLETE;
                        s_context.finger_count = 0;
                        s_context.registerd_count = 0;
                        stop();
                    }
                } else if(result == MDLAUTH_REG_MISSMATCH){
                	DBGLOG0("dbg_MDLAUTH_REG_MISSMATCH");
                    callback_type = MDLAUTH_EVT_REGISTRATION_COMPLETE;
                    s_context.finger_count = 0;
                    s_context.registerd_count = 0;
                    stop();
        ...
        case MSG_CANCEL: {
        	DBGLOG0("dbg_MSG_CANCEL");	//dbgmaru
            stop();
...
{
    assert(s_context.state == STATE_INITIALIZED); //dbgmaru
...
/*
    char capname[30] = {0};
	sprintf(capname, "cap%d.raw",(int)tim1);
	dly_tsk(100);
	cmndbg_hexdump(image.pixels, image.length, capname);
*/
...
int perform_registration(int index)
{
	...
	void* buf = NULL;
	size_t buffer_size = 0;
//	DBGLOG0("aquire_camera");	//dbgmaru
	buffer_size = cmnbuf_aquire_buffer(&buf, CMNBUF_USE_CAMERA_CAPTURE);    // キャプチャ用
	assert(buf);
	assert(buffer_size >= DRVCAM_CAPTURE_BUFFER_SIZE);
//	DBGLOG1("cap_size = %d",buffer_size);
	s_context.buffers.camera = buf;
	buf = NULL;
//    for(int i = 0; i < DRVCAM_CAPTURE_BUFFER_SIZE; i++ ){ //dbgmaru
//		s_context.buffers.camera[i] = 1;
//    }
	...
	MDLAUTHLIB_IMAGE_T image = {0};
	//maruhashi追加
	MDLAUTHLIB_RECT_T rect = (MDLAUTHLIB_RECT_T){FIN_X, FIN_Y, FIN_CUT_SIZE_X, FIN_CUT_SIZE_Y};
	if(s_context.registerd_count == 0){
//		DBGLOG1("camera address = %x",&s_context.buffers.camera); //dbgmaru
//		DBGLOG1("image address = %x",&image); //dbgmaru
		capture_image(&image);
		if(mdlauthlib_finger_search(&image, &rect)) {
/*
			aplui_dbg_set_led_pattern(1, 2);	//B
			aplui_dbg_set_led_pattern(2, 2);	//G
			aplui_dbg_set_led_pattern(0, 2); //R

			sprintf(finname, "0_reg1_fin%d.raw",s_context.finger_count);
			dly_tsk(100);
			cmndbg_hexdump(image.pixels, image.length, finname);
*/
			// 撮影バッファ開放
//			DBGLOG0("release_camera");	//dbgmaru
			cmnbuf_release_buffer(s_context.buffers.camera, CMNBUF_USE_CAMERA_CAPTURE);
			s_context.buffers.camera = NULL;
			error = MDLAUTH_REG_FIN_NG;
			return error;
		}
		else {
/*
			...
			aplui_dbg_set_led_pattern(0, 2); //R
			dly_tsk(100);
			cmndbg_hexdump(image.pixels, image.length, "0_reg1.raw");
*/
			// １回目画像処理実行
//			DBGLOG0("aquire_auth1");	//dbgmaru
			 buffer_size = cmnbuf_aquire_buffer(&buf, CMNBUF_USE_AUTHDATA_1);	// 認証用1
			assert(buf);
			assert(buffer_size >= sizeof(BUF_AUTHDATA1_T));
			DBGLOG1("auth1_size = %d",buffer_size);
			s_context.buffers.auth1 = buf;
			buf = NULL;
//		  	for(int i = 0; i < sizeof(BUF_AUTHDATA1_T); i++ ){	//dbgmaru
//		  		s_context.buffers.auth1->captured.data[i] = 1;
//		  	}

//			DBGLOG1("camera address = %x",&s_context.buffers.camera); //dbgmaru
//			DBGLOG1("auth1 address = %x",&s_context.buffers.auth1); //dbgmaru
//			DBGLOG1("image address = %x",&image); //dbgmaru
			mdlauthlib_registered_first_image(&(s_context.buffers.auth1->captured), &image);

			// 撮影バッファ開放
//			DBGLOG0("release_camera");	//dbgmaru
			cmnbuf_release_buffer(s_context.buffers.camera, CMNBUF_USE_CAMERA_CAPTURE);
			s_context.buffers.camera = NULL;
			error = MDLAUTH_REG_FIN_OK;
			return error;
		}
	} else{
		// 2回目画像取得
//		DBGLOG1("camera address = %x",&s_context.buffers.camera); //dbgmaru
//		DBGLOG1("image address = %x",&image); //dbgmaru
//		cmndbg_hexdump(&(s_context.buffers.auth1->captured), 120 * 60, "1_auth1.raw"); //dbgmaru
		capture_image(&image);
		if(mdlauthlib_finger_search(&image, &rect)) {
/*
		    aplui_dbg_set_led_pattern(1, 2);	//B
			aplui_dbg_set_led_pattern(2, 2);	//G
			aplui_dbg_set_led_pattern(0, 2); //R
			sprintf(finname, "0_reg2_fin%d.raw",s_context.finger_count);
			dly_tsk(100);
			cmndbg_hexdump(image.pixels, image.length, finname);
*/
			// 撮影バッファ開放
...
/*
			...
			aplui_dbg_set_led_pattern(0, 2); //R
			dly_tsk(100);
			cmndbg_hexdump(image.pixels, image.length, "0_reg2.raw");
*/
//			DBGLOG1("camera address = %x",&s_context.buffers.camera); //dbgmaru
//			DBGLOG1("auth1 address = %x",&s_context.buffers.auth1); //dbgmaru
//			DBGLOG1("buffers.auth1->captured = %x", s_context.buffers.auth1->captured); //dbgmaru
			// 2回目画像処理実行（登録判定も行う）
			uint8_t regsuccess = mdlauthlib_registered_second_image(&(s_context.buffers.auth1->captured), &image);
			if(regsuccess != 1) {
				 // 作業バッファ開放
//				DBGLOG0("release_auth1");	//dbgmaru
				cmnbuf_release_buffer(s_context.buffers.auth1, CMNBUF_USE_AUTHDATA_1);
				s_context.buffers.auth1 = NULL;
				// 撮影バッファ開放
//				DBGLOG0("release_camera");	//dbgmaru
				cmnbuf_release_buffer(s_context.buffers.camera, CMNBUF_USE_CAMERA_CAPTURE);
				s_context.buffers.camera = NULL;
				error = MDLAUTH_REG_MISSMATCH;
				return error;
			}

			// 画像処理実行
			mdlauthlib_regprocess_image(&(s_context.buffers.auth1->captured), &image);
			// 撮影バッファ開放
//			DBGLOG0("release_camera");	//dbgmaru
			cmnbuf_release_buffer(s_context.buffers.camera, CMNBUF_USE_CAMERA_CAPTURE);
			s_context.buffers.camera = NULL;
			// 作業バッファ取得
//			DBGLOG0("aquire_auth2"); //dbgmaru
			buffer_size = cmnbuf_aquire_buffer(&buf, CMNBUF_USE_AUTHDATA_2);	// 認証用2
			assert(buf);
			assert(buffer_size >= sizeof(BUF_AUTHDATA2_T));
			s_context.buffers.auth2 = buf;
...
//			DBGLOG1("write in %dms", systim2 - systim1);
			...
			DBGLOG0("release_auth1");	//dbgmaru
			cmnbuf_release_buffer(s_context.buffers.auth1, CMNBUF_USE_AUTHDATA_1);
//			cmndbg_hexdump(s_context.buffers.auth1, sizeof(BUF_AUTHDATA1_T), "auth1"); //dbgmaru
			s_context.buffers.auth1 = NULL;

			DBGLOG0("release_auth2");	//dbgmaru
			cmnbuf_release_buffer(s_context.buffers.auth2, CMNBUF_USE_AUTHDATA_2);
//			cmndbg_hexdump(s_context.buffers.auth2, DRVCAM_CAPTURE_BUFFER_SIZE, "auth2"); //dbgmaru
			s_context.buffers.auth2 = NULL;
